<?php
/** Database driver; G P S => Gangadhar Pranay Shashi   */
class GPS_db
{
    ...
    public static function get_instance($params = false)
    {
        ...
        if (!isset(self::$_instance[$instance_name]) or null === self::$_instance[$instance_name])
        {
            if (!is_array($params))
            {
                $dbuser = GPS_config::$dbuser;
                $dbpass = GPS_config::$dbpass;
                $dbname = GPS_config::$dbname;
                $dbhost = GPS_config::$dbhost;
                $dbencoding = GPS_config::$dbencoding;
            }
    ...
    private function __construct($dbuser, $dbpass, $dbname, $dbhost, $dbencoding)
    {
        ...
        if (GPS_config::$db_time_zone)
            $this->connect->query('SET time_zone = \'' . GPS_config::$db_time_zone . '\'');
    }
    ...
    private function error($text = 'Error!')
    {
        exit('<div class="gps-error" style="position:relative;line-height:1.25;padding:15px;color:#BA0303;margin:10px;border:1px solid #BA0303;border-radius:4px;font-family:Arial,sans-serif;background:#FFB5B5;box-shadow:inset 0 0 80px #E58989;">
            <span style="position:absolute;font-size:10px;bottom:3px;right:5px;">G P S</span>Please Reload / Refresh the Page</div>');
    }
}
