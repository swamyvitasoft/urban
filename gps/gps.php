...
define('GPSPATH', str_replace('\\', '/', dirname(__file__))); // str_replace - windows trick
require (GPSPATH . '/gps_config.php'); // configuration
require (GPSPATH . '/gps_db.php'); // database class
mb_internal_encoding(GPS_config::$mbencoding); // sets multibyte encoding globaly
date_default_timezone_set(@date_default_timezone_get()); // gps code not depends on timezone, but this can fix some warnings
...
class GPS
{
    ...
    protected $load_view = array(
        'list' => 'gps_list_view.php',
        'create' => 'gps_detail_view.php',
        'edit' => 'gps_detail_view.php',
        'view' => 'gps_detail_view.php');
    ...
    /** constructor, sets basic gps vars (they can be changed by public pethods) */
    protected function __construct()
    {
        GPS_config::$scripts_url = self::check_url(GPS_config::$scripts_url, true);
        GPS_config::$editor_url = self::check_url(GPS_config::$editor_url);
        GPS_config::$editor_init_url = self::check_url(GPS_config::$editor_init_url);

        $this->limit = GPS_config::$limit;
        $this->limit_list = GPS_config::$limit_list;
        $this->column_cut = GPS_config::$column_cut;
        $this->show_primary_ai_field = GPS_config::$show_primary_ai_field;
        $this->show_primary_ai_column = GPS_config::$show_primary_ai_column;

        $this->benchmark = GPS_config::$benchmark;
        $this->start_minimized = GPS_config::$start_minimized;
        $this->remove_confirm = GPS_config::$remove_confirm;
        $this->upload_folder_def = GPS_config::$upload_folder_def;

        $this->theme = GPS_config::$theme;
        $this->is_print = GPS_config::$enable_printout;
        $this->is_title = GPS_config::$enable_table_title;
        $this->is_csv = GPS_config::$enable_csv_export;
        $this->is_numbers = GPS_config::$enable_numbers;
        $this->is_pagination = GPS_config::$enable_pagination;
        $this->is_search = GPS_config::$enable_search;
        $this->is_limitlist = GPS_config::$enable_limitlist;
        $this->is_sortable = GPS_config::$enable_sorting;

        $this->language = GPS_config::$language;

        $this->search_pattern = GPS_config::$search_pattern;

        $this->demo_mode = GPS_config::$demo_mode;

        $this->default_tab = GPS_config::$default_tab;
        $this->prefix = GPS_config::$dbprefix;
        $this->is_rtl = GPS_config::$is_rtl;

        $this->strip_tags = GPS_config::$strip_tags;
        $this->safe_output = GPS_config::$safe_output;

        $this->lists_null_opt = GPS_config::$lists_null_opt;

        $this->date_format = array('php_d' => GPS_config::$php_date_format, 'php_t' => GPS_config::$php_time_format);
    }
    ...
    {
        if (isset($_POST['gps']['instance']) && isset($_POST['gps']['key']) && isset($_POST['gps']['task']))
        {
            self::init_prepare('post');
            $key = $_POST['gps']['key'] ? $_POST['gps']['key'] : self::error('Security key cannot be empty');
            $inst_name = $_POST['gps']['instance'] ? $_POST['gps']['instance'] : self::error('Instance name cannot be empty');
            $is_get = false;
        }
        elseif (isset($_GET['gps']['instance']) && isset($_GET['gps']['key']) && isset($_GET['gps']['task']) && $_GET['gps']['task'] ==
            'file')
        {
            self::init_prepare('get');
            $key = $_GET['gps']['key'] ? $_GET['gps']['key'] : self::error('Security key cannot be empty');
            $inst_name = $_GET['gps']['instance'] ? $_GET['gps']['instance'] : self::error('Instance name cannot be empty');
            $is_get = true;
        }
        else
        {
        ...
        if (isset($_SESSION['lists']['gps_session'][$inst_name]['key']) && $_SESSION['lists']['gps_session'][$inst_name]['key'] ==
            $key)
        {
            ...
            self::error('<div align="center"><strong>Just reload the page, nothing happened :)</strong></div>');
    }
    protected static function init_prepare($method = false)
    {
        ...
        {
            case 'post':
                $sess_name = (GPS_config::$dynamic_session && isset($_POST['gps']['sess_name']) && $_POST['gps']['sess_name']) ? $_POST['gps']['sess_name'] :
                    GPS_config::$sess_name;
                break;
            case 'get':
                $sess_name = (GPS_config::$dynamic_session && isset($_GET['gps']['sess_name']) && $_GET['gps']['sess_name']) ? $_GET['gps']['sess_name'] :
                    GPS_config::$sess_name;
                break;
            default:
                $sess_name = GPS_config::$sess_name;
                break;
        }
        self::session_start($sess_name);
        if (is_callable(GPS_config::$before_construct))
        {
            call_user_func(GPS_config::$before_construct);
        }
    }
    ...
    public static function session_start($sess_name = false)
    {
        ...
        if (!session_id() && !GPS_config::$external_session && !GPS_config::$alt_session)
        {
            if (!headers_sent())
            {
                session_name($sess_name);
                session_cache_expire(GPS_config::$sess_expire);
                session_set_cookie_params(0, '/');
                session_start();
            }
            else
                self::error('Gps can not create session, because the output is already sent into browser. 
                Try to define Gps instance before the output start or use session_start() at the beginning of your script');
        }
        if (GPS_config::$alt_session)
        {
            if (!headers_sent())
            {
                ...
                setcookie($sess_name, self::$sess_id, time() + GPS_config::$alt_lifetime * 60, '/');
            }
            else
                self::error('Gps can not start session, because the output is already sent into browser. 
                Try to define Gps instance before the output start or use <strong>GPS::session_start();</strong> at the beginning of your script');
        }
    }
    ...
    public function nested_table($instance_name = '', $field = '', $inner_tbl = '', $tbl_field = '')
    {
        if ($instance_name && $field && $inner_tbl && $tbl_field)
        {
            ...
            foreach ($fdata as $fitem)
            {
                ...
                $instance = GPS::get_instance($instance_name); // just another gps object
                ...
                switch ($type)
                {
                    ...
                    case 'point':
                        ...
                        $def_attr = array( // defaults
                            'text' => GPS_config::$default_text,
                            'search_text' => GPS_config::$default_search_text,
                            'zoom' => GPS_config::$default_zoom,
                            'width' => GPS_config::$default_width,
                            'height' => GPS_config::$default_height,
                            'search' => GPS_config::$default_coord,
                            'coords' => GPS_config::$default_search);
                        $this->field_attr[$fitem['table'] . '.' . $fitem['field']] = array_merge($def_attr, (array )$attr);
                        break;
                    case 'remote_image':
    ...
    protected function _run_task()
    {
        ...
        switch ($this->task)
        {
            ...
            case 'print':
                ...
                $this->_set_field_types('list', GPS_config::$print_all_fields);
                $this->theme = 'printout';
                return $this->_list();
                break;
            case 'depend':
            ...
            case 'file':
                ...
                break;
            case 'csv':
                $this->_set_field_types('list', GPS_config::$csv_all_fields);
                return $this->_csv();
                break;
            case 'list':
            default:
    ...
    protected function render_custom_csv()
    {
        ...
        fputcsv($output, $this->columns_names, GPS_config::$csv_delimiter, GPS_config::$csv_enclosure);
        ...
        while ($row = $db->result->fetch_assoc()) // low level result process, saves memory
        {
            ...
            fputcsv($output, $out, GPS_config::$csv_delimiter, GPS_config::$csv_enclosure);
        }
    }
    ...
    protected function render_output()
    {
        ...
        {
            $contents = '';
            if (!self::$css_loaded && !GPS_config::$manual_load)
            {
                $contents .= self::load_css();
            }
            ob_start();
            include (GPSPATH . '/' . GPS_config::$themes_path . '/' . $this->theme . '/gps_container.php');
            $contents .= ob_get_contents();
            ob_end_clean();
            unset($this->data);
            if (!self::$js_loaded && !GPS_config::$manual_load)
            {
    ...
    protected function after_render()
    {
        ...
        if (is_callable(GPS_config::$after_render))
        {
            call_user_func(GPS_config::$after_render);
        }
    }
    ...
    public function _csv()
    {
        ...
        $output = fopen('php://output', 'w');
        fwrite($output, chr(0xEF) . chr(0xBB) . chr(0xBF)); // bom
        fputcsv($output, $headers, GPS_config::$csv_delimiter, GPS_config::$csv_enclosure);
    ...
    {
        if (isset($_POST['gps'][$field]))
        {
            if (get_magic_quotes_gpc())
            {
                if (is_array($_POST['gps'][$field]))
                {
                    array_walk_recursive($_POST['gps'][$field], array($this, 'stripslashes_callback'));
                }
                else
                {
                    $_POST['gps'][$field] == stripslashes($_POST['gps'][$field]);
                }
            }
            if (GPS_config::$auto_xss_filtering)
            {
            ...
            if (($field == 'postdata' or $field == 'unique') && $_POST['gps'][$field])
            {
                $data_keys = array_keys($_POST['gps'][$field]);
                foreach ($data_keys as $k => $key)
                {
                    $data_keys[$k] = $xss ? $xss->xss_clean($this->fieldname_decode($key)) : $this->fieldname_decode($key);
                    if ($xss)
                    {
                        $_POST['gps'][$field][$key] = $xss->xss_clean($_POST['gps'][$field][$key]);
                    }
                }
                return array_combine($data_keys, $_POST['gps'][$field]);
            }
            elseif ($filter)
            {
                switch ($filter)
                {
                    case 'key':
                        return str_replace('`', '', $xss ? $xss->xss_clean($_POST['gps'][$field]) : $_POST['gps'][$field]);
                        break;
                    case 'int':
                        return (int)$_POST['gps'][$field];
                        break;
                    case 'trim':
                        return trim($xss ? $xss->xss_clean($_POST['gps'][$field]) : $_POST['gps'][$field]);
                        break;
                    case 'base64':
                        return $xss ? $xss->xss_clean($this->fieldname_decode($_POST['gps'][$field])) : $this->fieldname_decode($_POST['gps'][$field]);
                        break;
                    default:
                        return $xss ? $xss->xss_clean($_POST['gps'][$field]) : $_POST['gps'][$field];
                        break;
                }
            }
            else
    ...
    {
        if (isset($_GET['gps'][$field]))
        {
            if (get_magic_quotes_gpc())
            {
                if (is_array($_GET['gps'][$field]))
                {
                    array_walk_recursive($_GET['gps'][$field], array($this, 'stripslashes_callback'));
                }
                else
                {
                    $_GET['gps'][$field] == stripslashes($_GET['gps'][$field]);
                }
            }
            if (GPS_config::$auto_xss_filtering)
            {
            ...
            if ($filter)
            {
                ...
                {
                    case 'key':
                        return str_replace('`', '', $xss ? $xss->xss_clean($_GET['gps'][$field]) : $_GET['gps'][$field]);
                        break;
                    case 'int':
                        return (int)$_GET['gps'][$field];
                        break;
                    case 'trim':
                        return trim($xss ? $xss->xss_clean($_GET['gps'][$field]) : $_GET['gps'][$field]);
                        break;
                    default:
                        return $xss ? $xss->xss_clean($_GET['gps'][$field]) : $_GET['gps'][$field];
                        break;
                }
            }
            else
    ...
    protected function _create($postdata = array())
    {
        ...
        if ($this->before_create)
        {
            ...
            {
                $postdata = new GPS_postdata($this->result_row, $this);
    ...
    protected function _save()
    {
        ...
        if (!$this->primary_val)
        {
            ...
            $pd = new GPS_postdata($postdata, $this);
            ...
            if ($this->alert_create)
            {
                foreach ($this->alert_create as $alert)
                {
                    ...
                    if (GPS_config::$email_enable_html)
                        $message = $alert['message'] . '<br /><br />' . "\r\n" . ($alert['link'] ? '<a href="' . $alert['link'] .
                            '" target="_blank">' . $alert['link'] . '</a>' : '');
                    else
                        $message = $alert['message'] . "\r\n\r\n" . ($alert['link'] ? $alert['link'] : '');
                    $this->send_email($send_to, $alert['subject'], $message, $alert['cc'], GPS_config::$email_enable_html);
                }
            }
            if ($this->mass_alert_create)
            {
                foreach ($this->mass_alert_create as $alert)
                {
                    ...
                    {
                        $this->send_email($row[$alert['email_column']], $alert['subject'], $message, array(), GPS_config::$email_enable_html);
                    }
                }
            }
    ...
    protected function _receive_post($task = false, $primary = false)
    {
        ...
        else
        {
            ...
            $this->limit = $this->_post('limit', ($this->limit ? $this->limit : GPS_config::$limit));
        ...
        if (is_array($this->table_info) && count($this->table_info))
        {
            ...
            foreach ($this->table_info as $table => $types)
            {
                foreach ($types as $row)
                {
                    ...
                    if (!$this->field_null[$field_index] && GPS_config::$not_null_is_required && !isset($this->validation_required[$field_index]))
                    {
    ...
    protected function _define_field_type($row, $field_index)
    {
        ...
        switch ($type)
        {
            ...
            case 'boolean':
                ...
                if ($max_l == 1 && GPS_config::$make_checkbox)
                {
            ...
            case 'longtext':
                ...
                if (!isset($this->no_editor[$field_index]) && GPS_config::$auto_editor_insertion)
            ...
            case 'enum':
                ...
                $this->field_type[$field_index] = GPS_config::$enum_as_radio ? 'radio' : 'select';
            ...
            case 'set':
                ...
                $this->field_type[$field_index] = GPS_config::$set_as_checkboxes ? 'checkboxes' : 'multiselect';
                $this->field_attr[$field_index]['values'] = $max_l;
                if (!isset($this->defaults[$field_index]))
                    $this->defaults[$field_index] = $row['Default'];
                break;
            case 'point':
                ...
                if (!isset($this->defaults[$field_index]))
                    $this->defaults[$field_index] = GPS_config::$default_point ? GPS_config::$default_point : '0,0';
                break;
        }
    }
    ...
    protected function _render_list()
    {
        ...
        $mode = 'list';
        $view_file = GPSPATH . '/' . GPS_config::$themes_path . '/' . $this->theme . '/' . $this->load_view['list'];
    ...
    protected function _render_details($mode)
    {
        ...
        */
        ...
        if ($this->inner_table_instance && ($mode == 'view' or $mode == 'edit')) // restoring nested objects
        {
            foreach ($this->inner_table_instance as $inst_name => $field)
            {
                if (isset($this->result_row[$field]))
                {
                    ...
                    if ($mode == 'view' && GPS_config::$nested_readonly_on_view)
                    {
                    ...
                    $this->nested_rendered[$inst_name] =
                        '<div class="gps-nested-container gps-container"><div class="gps-ajax" id="gps-ajax-' . base_convert(rand(), 10,
                        36) . '">' . $instance->render('list') . '</div></div>';
                }
            }
        }

        $view_file = GPSPATH . '/' . GPS_config::$themes_path . '/' . $this->theme . '/' . $this->load_view[$mode];
        $this->check_file($view_file, 'render');
        ob_start();
        include ($view_file);
        ...
        //$this->data .= '<div class="gps-nested-container gps-container"><div class="gps-ajax" id="gps-ajax-' .
        //    base_convert(rand(), 10, 36) . '">' . $instance->render('list') . '</div></div>';
        $this->nested_rendered[$inst_name] = '<div class="gps-nested-container gps-container"><div class="gps-ajax" id="gps-ajax-' .
        base_convert(rand(), 10, 36) . '">' . $instance->render('list') . '</div></div>';
        }
        }
        }*/
    ...
    protected function _pagination($total, $start, $limit, $numpos = 10, $numlr = 2)
    {
        if ($total > $limit)
        {
            ...
            '"><a href="javascript:;" class="gps-action" data-start="' . $limit1 . '">' . $i . '</a></li>';
            }
            }*/
    ...
    protected function _cut($string, $field, $wordsafe = true, $dots = true)
    {
        ...
        $string = html_entity_decode($string, ENT_QUOTES, GPS_config::$mbencoding);
        ...
        $slen = mb_strlen($strip_string, GPS_config::$mbencoding);
        if ($slen <= $len || (GPS_config::$print_full_texts && $this->theme == 'printout'))
        {
            return $this->output_string($string, $this->strip_tags, $safe);
        }
        if ($wordsafe)
        {
            $end = $len;
            while ((mb_substr($strip_string, --$len, 1, GPS_config::$mbencoding) != ' ') && ($len > 0))
            {
            }
            if ($len == 0)
            {
            ...
            return $this->output_string(mb_substr($strip_string, 0, $len, GPS_config::$mbencoding), false, $safe) . ($dots ?
                '&#133;' : '');
        }
        return $this->output_string(mb_substr($strip_string, 0, $len, GPS_config::$mbencoding), false, $safe) . ($dots ?
            '&#133;' : '');
    ...
    {
        return mb_convert_case(str_replace('_', ' ', $text), MB_CASE_TITLE, GPS_config::$mbencoding);
    }
    ...
    public function _export_vars()
    {
        ...
        {
            if (isset($_SESSION['lists']['gps_session']) && $_SESSION['lists']['gps_session'])
            {
                foreach ($_SESSION['lists']['gps_session'] as $s_key => $s_val)
                { // workaround on some servers session duplication
                    $old_time = isset($s_val['time']) ? (int)$s_val['time'] : 0;
                    if ($time > $old_time + GPS_config::$autoclean_timeout) // autocleaner

                        unset($_SESSION['lists']['gps_session'][$s_key]);
                }
            }
        }
        ...
        {
            $_SESSION['lists']['gps_session'][$inst_name][$item] = $this->{$item};
        }
        $_SESSION['lists']['gps_session'][$inst_name]['before'] = $this->find_prev_task();

        if (GPS_config::$alt_session)
        {
            $data = $this->encrypt($_SESSION['lists']['gps_session']);

            if (class_exists('Memcache'))
            {
                $mc = new Memcache();
                $mc->connect(GPS_config::$mc_host, GPS_config::$mc_port);
                $res = $mc->set(self::$sess_id, $data, false, GPS_config::$alt_lifetime * 60);
            }
            elseif (class_exists('Memcached'))
            {
                $mc = new Memcached();
                $mc->connect(GPS_config::$mc_host, GPS_config::$mc_port);
                $res = $mc->set(self::$sess_id, $data, GPS_config::$alt_lifetime * 60);
            }
            else
            {
                self::error('Can\'t use alternative session. Memcache(d) is not available');
            }
            unset($_SESSION['lists']['gps_session']);
            if (!$res)
            {
    ...
    public function import_vars($key = false)
    {
        if (GPS_config::$alt_session)
        {
            ...
            $_SESSION['lists']['gps_session'] = $this->decrypt($data[0], $data[1]);
            unset($data);
            if (!$_SESSION['lists']['gps_session'])
            {
        ...
        foreach ($this->params2save() as $item)
        {
            $this->{$item} = $_SESSION['lists']['gps_session'][$inst_name][$item];
        }
    ...
    {
        return '<span class="gps-none">' . $value . '</span>';
    }
    ...
    {
        if (GPS_config::$clickable_list_links)
        {
    ...
    protected function create_radio($name, $value = '', $tag = array())
    {
        ...
        $label_tag = array('tag' => 'label', 'class' => 'gps-radio-label');
    ...
    protected function create_checkboxes($name, $value = '', $tag = array())
    {
        ...
        $label_tag = array('tag' => 'label', 'class' => 'gps-checkboxes-label');
    ...
    protected function create_hidden($name, $value = '', $tag = array())
    {
        return $this->single_tag($tag + array(
            ...
            'type' => 'hidden',
            'value' => $value,
            'name' => $name), 'gps-input', $this->field_attr[$name], true);
    }
    ...
    protected function create_file($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        $out .= $this->open_tag('div', 'gps-upload-container'); // file and delete button
        $out .= $this->open_tag('div', $this->theme_config('grid_button_group'));
        if ($value)
        {
            //$out .= $this->open_tag('span', 'gps-file-name');
        ...
        {
            $out .= $this->open_tag('span', 'gps-nofile ' . $this->theme_config('no_file'));
        ...
        if (!isset($tag['readonly']) && !isset($tag['disabled']))
        {
            ...
            $attr = array(
                ...
                'value' => $value,
                'type' => 'hidden');
            $out .= $this->single_tag('input', 'gps-input', $attr, true);
            // upload button
    ...
    {
        $out = '';
        $out .= $this->open_tag('span', $this->theme_config('upload_button'), array('class' => 'gps-add-file'));
    ...
    protected function create_view_file($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        {
            $out .= $this->open_tag('span', 'gps-nofile');
            $out .= $this->lang('no_file') . $this->close_tag('span');
        }
        return $out;
    }
    protected function create_image($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        $out = ''; // upload container
        $out .= $this->open_tag('div', 'gps-upload-container'); // image and delete button
        if ($value)
        {
            $binary = isset($this->upload_config[$name]['blob']) ? true : false;
            if ($binary && !$is_upload)
        ...
        {
            $out .= $this->open_tag('div', $this->theme_config('grid_button_group'));
            $out .= $this->open_tag('span', 'gps-noimage ' . $this->theme_config('no_file'));
    ...
    protected function create_view_image($name, $value = '', $tag = array())
    {
        ...
        {
            $out .= $this->open_tag('span', 'gps-noimage');
            $out .= $this->lang('no_image') . $this->close_tag('span');
        }
    ...
    protected function create_point($name, $value = '', $tag = array())
    {
        ...
        if (!$value)
        {
            $value = GPS_config::$default_point ? GPS_config::$default_point : '0,0';
        }
    ...
    protected function create_view_point($name, $value = '', $tag = array())
    {
        ...
        if (!$value)
        {
            $value = GPS_config::$default_point;
        }
        if ($value)
        {
            $tag = array(
    ...
    {
        exit('<div class="gps-error" style="position:relative;line-height:1.25;padding:15px;color:#BA0303;margin:10px;border:1px solid #BA0303;border-radius:4px;font-family:Arial,sans-serif;background:#FFB5B5;box-shadow:inset 0 0 80px #E58989;">
            <span style="position:absolute;font-size:10px;bottom:3px;right:5px;">Gps</span>' . $text . '</div>');
    }
    ...
    protected function _upload_file()
    {
        ...
        if (isset($_FILES) && isset($_FILES['gps-attach']) && !$_FILES['gps-attach']['error'])
        {
            $file = $_FILES['gps-attach'];
    ...
    {
        $out = ''; // upload container
        $out .= $this->open_tag('div', 'gps-upload-container');
        ...
        $attr = array(
            ...
            'alt' => '');
        $out .= $this->single_tag('img', 'gps-crop', $attr);
    ...
    protected function _compare($val1, $operator, $val2)
    {
        switch ($operator)
        {
            ...
            case '!=':
                return ($val1 != $val2) ? true : false;
            case '^=':
                return (mb_strpos($val1, $val2, 0, GPS_config::$mbencoding) === 0) ? true : false;
            case '$=':
                return (mb_strpos($val1, $val2, 0, GPS_config::$mbencoding) == (mb_strlen($val1, GPS_config::$mbencoding) -
                    mb_strlen($val2, GPS_config::$mbencoding))) ? true : false;
            case '~=':
                return (mb_strpos($val1, $val2, 0, GPS_config::$mbencoding) !== false) ? true : false;
            default:
                return false;
        }
    }
    protected function create_modal($field, $content, $image = false)
    {
        ...
        $out .= $this->open_tag('a', 'gps_modal', $attr);
        if (GPS_config::$images_in_grid && $image)
        {
            $out .= $content;
        }
        else
                    ...
                    if ($value)
                    {
                        ...
                        if($ext == 'jpeg' || $ext == 'jpg' || $ext == 'png' || $ext == 'gif')
                        {
                            $out .= $this->single_tag('img', '', array(
                                'alt' => '',
                                'src' => file_exists($this->upload_folder_def.'/small/'.$value) ? $this->upload_folder_def.'/small/'.$value : $this->upload_folder_def.'/'.$value,
                                'style' => 'max-height: ' . GPS_config::$images_in_grid_height . 'px;'));
                        } 
                        elseif($ext == 'mp4')
                        {
                            $height = GPS_config::$images_in_grid_height;
                            $max_height = $height*2;
                            $out .= '<a target="_blank" href="' .$this->upload_folder_def.'/'.$value. '">'.$value.'</a>';
                        }
                        elseif(strpos($frame,'youtu.be') !== false)
                        {
                            $height = GPS_config::$images_in_grid_height;
                            $max_height = $height*2;
                            $link = (strpos($frame, '?') !== false)?str_replace('youtu.be/','youtube.com/embed/',$value).'&autoplay=0&showinfo=0&controls=0':str_replace('youtu.be/','youtube.com/embed/',$value).'?autoplay=0&showinfo=0&controls=0';
                            $out .= '<a target="_blank" href="' .$link. '">'.$value.'</a>';
                        }
                        else
                        {
    ...
    protected function file_link($field, $primary_val, $thumb = false, $crop = false, $filename = false)
    {
        ...
        if ($thumb !== false)
        {
            $params['gps']['thumb'] = $thumb;
        }
        if ($crop)
        {
            $params['gps']['crop'] = $crop;
        }
        if (GPS_config::$dynamic_session)
        {
            $params['gps']['sess_name'] = session_name();
        }
        return GPS_config::$scripts_url . '/' . GPS_config::$ajax_uri . '?' . http_build_query($params);
    }
    ...
    {
        return htmlspecialchars((string )$text, ENT_QUOTES, GPS_config::$mbencoding);
    }
    ...
    protected function send_email($to, $subject = '(No subject)', $message = '', $cc = array(), $html = true)
    {
        $header = 'MIME-Version: 1.0' . "\r\n" . 'Content-type: text/' . ($html ? 'html' : 'plain') . '; charset=UTF-8' . "\r\n" .
            'From: ' . GPS_config::$email_from_name . ' <' . GPS_config::$email_from . ">\r\n";
        if ($cc)
    ...
    protected function _cell_attrib($field, $value, $order, &$row, $is_sum = false, $row_color = false, $row_class = false)
    {
        ...
        if ($field == $order && $this->is_sortable)
            $column_class[] = 'gps-current';
        if ($is_sum)
            $column_class[] = 'gps-sum';
        if ($row_color)
        {
    ...
    {
        if (is_file(GPSPATH . '/' . GPS_config::$lang_path . '/' . $this->language . '.ini'))
            self::$lang_arr = parse_ini_file(GPSPATH . '/' . GPS_config::$lang_path . '/' . $this->language . '.ini');
        elseif (is_file(GPSPATH . '/' . GPS_config::$lang_path . '/en.ini'))
            self::$lang_arr = parse_ini_file(GPSPATH . '/' . GPS_config::$lang_path . '/en.ini');
        if ($this->set_lang)
        {
    ...
    {
        if (is_file(GPSPATH . '/' . GPS_config::$lang_path . '/' . GPS_config::$language . '.ini'))
            self::$lang_arr = parse_ini_file(GPSPATH . '/' . GPS_config::$lang_path . '/' . GPS_config::$language . '.ini');
        elseif (is_file(GPSPATH . '/' . GPS_config::$lang_path . '/en.ini'))
            self::$lang_arr = parse_ini_file(GPSPATH . '/' . GPS_config::$lang_path . '/en.ini');
    }
    protected function _get_theme_config()
    { // loads theme configuration from ini file
        if (is_file(GPSPATH . '/' . GPS_config::$themes_path . '/' . $this->theme . '/gps.ini'))
            $this->theme_config = parse_ini_file(GPSPATH . '/' . GPS_config::$themes_path . '/' . $this->theme . '/gps.ini');
        else
            self::error('gps.ini does not exist in your theme folder');
    }
    protected function lang($text = '')
    {
        $langtext = mb_convert_case($text, MB_CASE_LOWER, GPS_config::$mbencoding);
        return htmlspecialchars((isset(self::$lang_arr[$langtext]) ? self::$lang_arr[$langtext] : $text), ENT_QUOTES,
            GPS_config::$mbencoding);
    }
    protected function theme_config($text = '')
    {
        $text = mb_convert_case($text, MB_CASE_LOWER, GPS_config::$mbencoding);
        return htmlspecialchars((isset($this->theme_config[$text]) ? $this->theme_config[$text] : ''), ENT_QUOTES, GPS_config::
            $mbencoding);
    }
    ...
    public static function load_css()
    {
        ...
        if (self::$css_loaded)
        {
            self::error('GPS\'s styles already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
        }

        self::$css_loaded = true;
        if (GPS_config::$load_bootstrap)
        {
            $out .= '<link href="' . GPS_config::$scripts_url . '/' . GPS_config::$plugins_uri .
                '/bootstrap/css/bootstrap.min.css" rel="stylesheet" type="text/css" />';
        }
        if (GPS_config::$load_jquery_ui)
            $out .= '<link href="' . GPS_config::$scripts_url . '/' . GPS_config::$plugins_uri .
                '/jquery-ui/jquery-ui.min.css" rel="stylesheet" type="text/css" />';
        if (GPS_config::$load_jcrop)
            $out .= '<link href="' . GPS_config::$scripts_url . '/' . GPS_config::$plugins_uri .
                '/jcrop/jquery.Jcrop.min.css" rel="stylesheet" type="text/css" />';
        $out .= '<link href="' . GPS_config::$scripts_url . '/' . GPS_config::$plugins_uri .
            '/timepicker/jquery-ui-timepicker-addon.css" rel="stylesheet" type="text/css" /><link href="' . GPS_config::$scripts_url .
            '/' . GPS_config::$themes_uri . '/' . GPS_config::$theme . '/gps.css" rel="stylesheet" type="text/css" />';
        return $out;
    }
    public static function load_js()
    {
        ...
        if (self::$js_loaded)
        {
            self::error('GPS\'s scripts already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
        }
        self::$js_loaded = true;
        if (GPS_config::$load_jquery)
            $out .= '<script src="' . GPS_config::$scripts_url . '/' . GPS_config::$plugins_uri . '/jquery.min.js"></script>';
        if (GPS_config::$jquery_no_conflict)
        {
            $out .= '
        ...
        if (GPS_config::$load_jquery_ui)
            $out .= '<script src="' . GPS_config::$scripts_url . '/' . GPS_config::$plugins_uri .
        ...
        if (GPS_config::$editor_url)
            $out .= '<script src="' . GPS_config::$editor_url . '"></script>';
        if (GPS_config::$load_googlemap)
            $out .= '<script src="//maps.google.com/maps/api/js?key=AIzaSyADvgfPSrdO7rxZxkdfuMs8438tmFyhBvk&callback=initMap&language=' . $language . '"></script>';
        $out .= '<script src="' . GPS_config::$scripts_url . '/' . GPS_config::$plugins_uri . '/gps.js"></script>';

        $config = array(
            'url' => GPS_config::$scripts_url . '/' . GPS_config::$ajax_uri,
            'editor_url' => GPS_config::$editor_url,
            'editor_init_url' => GPS_config::$editor_init_url,
            'force_editor' => GPS_config::$force_editor,
            'date_first_day' => GPS_config::$date_first_day,
            'date_format' => GPS_config::$date_format,
            'time_format' => GPS_config::$time_format,
            'lang' => self::$lang_arr,
            'rtl' => GPS_config::$is_rtl ? 1 : 0);
        $out .= '
            <script type="text/javascript">
            <!--
            
           	var gps_config = ' . json_encode($config) . ';
                            
            -->
            </script>';
        if ($language != 'en')
        {
            if (is_file(GPSPATH . '/' . GPS_config::$lang_path . '/datepicker/jquery.ui.datepicker-' . $language . '.js'))
                $out .= '<script src="' . GPS_config::$scripts_url . '/' . GPS_config::$lang_uri .
                    '/datepicker/jquery.ui.datepicker-' . $language . '.js"></script>';
            if (is_file(GPSPATH . '/' . GPS_config::$lang_path . '/timepicker/jquery-ui-timepicker-' . $language . '.js'))
                $out .= '<script src="' . GPS_config::$scripts_url . '/' . GPS_config::$lang_uri .
    ...
    protected function get_limit_list($limit = 20, $buttons = false)
    {
        ...
        if ($buttons)
        {
            $out .= $this->open_tag(array(
                'tag' => 'div',
                'class' => 'btn-group gps-limit-buttons',
            ...
            foreach ($this->limit_list as $limts)
            {
                ...
                $out .= $this->open_tag(array(
                    ...
                    'data-limit' => $limts), 'gps-action') . $this->lang($limts) . $this->close_tag(array('tag' => 'button'));
            }
        ...
        else
        {
            $out .= $this->open_tag(array(
                ...
                'name' => 'limit'), 'gps-actionlist gps-data');
            foreach ($this->limit_list as $limts)
            {
    ...
    {
        if (!GPS_config::$performance_mode)
        {
    ...
    {
        if (!is_array($value) && !GPS_config::$performance_mode && $value)
        {
    ...
    protected function send_http_request($url, $data, $method, $return_result = false)
    {
        ...
        {
            case 'get':
                $ch = curl_init($path . ((mb_strpos($path, '?', 0, GPS_config::$mbencoding) === false) ? '?' : '&') . $data);
                break;
            case 'post':
                $ch = curl_init($path);
        ...
        if (GPS_config::$use_browser_info)
        {
        ...
        'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
    {
        ...
        {
            $tabs_header = $this->open_tag($tabs_block, $this->theme_config('tabs_container'), array('class' => 'gps-tabs')) . $this->
            ...
            if ($this->nested_rendered && GPS_config::$nested_in_tab)
            {
    ...
    protected function get_table_tooltip()
    {
        ...
        if ($this->table_tooltip)
        {
            ...
            $out .= $this->open_tag(array(
                'tag' => 'a',
                'href' => 'javascript:;',
                'class' => 'gps-tooltip gps-button-link',
    ...
    protected function render_search()
    {
        ...
        if ($this->is_search)
        {
            ...
            $attr = array('class' => 'gps-search-toggle', 'href' => 'javascript:;');
            if ($this->search or GPS_config::$search_opened)
            {
                $attr['style'] = 'display:none;';
            }
            $out .= $this->open_tag('a', $this->theme_config('search_open'), $attr);
            $out .= $this->lang('search') . $this->close_tag('a');
            $attr = array('class' => 'gps-search');
            if (!$this->search && !GPS_config::$search_opened)
            {
                $attr['style'] = 'display:none;';
            }
            ...
            $attr = array('class' => 'gps-data', 'name' => 'column');
            if ($this->search_default === false && !$this->search_columns)
            {
                $out .= $this->open_tag(array('tag' => 'input', 'type' => 'hidden'), 'gps-columns-select ' . $this->theme_config('search_fieldlist'),
                    $attr);
            }
            else
            {
    ...
    {
        $attr_preset = array('class' => 'gps-searchdata', 'name' => 'phrase');
        if ($field == $this->column)
        {
            $class = 'gps-search-active';
        }
        else
        {
            $class = '';
        ...
        switch ($attr['data-type'])
        {
            ...
            case 'timestamp':
                if (!isset($fieldlist['date']) or $field == $this->column)
                {
                    ...
                    $attr_range = array(
                        'class' => 'gps-daterange gps-searchdata ' . $this->theme_config('search_range'),
                        'name' => 'range',
                        'data-fieldtype' => 'date');
                    if ($field != $this->column)
                    {
                    ...
                    $fieldlist['date'] .= $this->open_tag('option', '', array('value' => '')) . $this->lang('choose_range') . $this->
                        close_tag('option');
                    if (GPS_config::$available_date_ranges)
                    {
                        foreach (GPS_config::$available_date_ranges as $range)
                        {
                    ...
                    $fieldlist['date'] .= $this->single_tag('input', 'gps-datepicker-from ' . $class . ' ' . $this->theme_config('search_from'),
                        $attr);
                    $attr['name'] = 'phrase][to';
                    $attr['value'] = (isset($phrase['to']) && $field == $this->column) ? $phrase['to'] : '';
                    $fieldlist['date'] .= $this->single_tag('input', 'gps-datepicker-to ' . $class . ' ' . $this->theme_config('search_to'),
                        $attr);
                }
                break;
            case 'select':
    ...
    {
        $out = '';
        $out .= $this->open_tag($row, 'gps-th');
        if ($this->is_numbers)
        {
            $out .= $this->open_tag($item, 'gps-num') . '&#35;' . $this->close_tag($item);
        }
        ...
        {
            $out .= $this->open_tag($item, 'gps-actions') . '&nbsp;' . $this->close_tag($item);
        }
        foreach ($this->columns as $field => $fitem)
        {
            if (isset($this->field_type[$field]) && ($this->field_type[$field] == 'password' or $this->field_type[$field] ==
            ...
            if ($this->is_sortable)
            {
                ...
                {
                    $class .= ' gps-current gps-' . $this->order_direct;
    ...
    protected function render_grid_body($row_tag = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        if ($this->result_list)
        {
            foreach ($this->result_list as $key => $row)
            {
                ...
                $out .= $this->open_tag($row_tag, 'gps-row gps-row-' . $i);
                if ($this->is_numbers)
                {
                    $out .= $this->open_tag($item, 'gps-num', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class)) . ($key +
                        $this->start + 1) . $this->close_tag($item);
                }
                if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->
                    grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'left')
                {
                    $out .= $this->open_tag($item, 'gps-actions', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class));
                ...
                if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->
                    grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'right')
                {
                    $out .= $this->open_tag($item, 'gps-actions' . (GPS_config::$fixed_action_buttons ? ' gps-fix' : ''), $this->
        ...
        else
        {
            ...
            $out .= $this->open_tag($row_tag, 'gps-row') . $this->open_tag($item, '', array('colspan' => $j)) . $this->lang('table_empty') .
    ...
    protected function render_grid_footer($row = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        {
            $out .= $this->open_tag($row, 'gps-tf');
            if ($this->is_numbers)
            {
                $out .= $this->open_tag($item, 'gps-num') . '&Sigma;' . $this->close_tag($item);
            }
    ...
    protected function render_benchmark($tag = array('tag' => 'span'))
    {
        if ($this->benchmark)
        {
            return $this->open_tag($tag, 'gps-benchmark') . $this->benchmark_end() . $this->close_tag($tag);
        }
        return '';
    }
    protected function render_control_fields()
    {
        ...
        $out .= $this->single_tag($tag, '', array('name' => 'limit', 'value' => ($this->limit ? $this->limit : GPS_config::$limit)));
    ...
    protected function render_button($name = '', $task = '', $after = '', $class = '', $icon = '', $mode = '', $primary = '')
    {
        ...
        if (!isset($this->hide_button[$name]))
        {
            ...
            $out .= $this->open_tag($tag, 'gps-action');
            if ($icon && !$this->is_rtl)
            {
    ...
    protected function csv_button($class = '', $icon = '')
    {
        ...
        {
            return $this->render_button('export_csv', 'csv', '', $class . ' gps-in-new-window', $icon);
        }
    }
    protected function print_button($class = '', $icon = '')
    {
        ...
        {
            return $this->render_button('print', 'print', '', $class . ' gps-in-new-window', $icon);
        }
    }
    ...
    protected function get_image_folder($field)
    {
        ...
        {
            $path = $this->check_folder(GPS_config::$upload_folder_def, 'get_image_folder');
        }
    ...
    {
        $range = array();
        $time = time() /* + 3600 * GPS_config::$local_time_correction*/;
        $week_day = date('w', $time) /* + GPS_config::$date_first_day*/;
        switch ($name)
        {
            default:
            case 'today':
            ...
            case 'this_week_today':
                if ($week_day >= GPS_config::$date_first_day)
                {
                    $offset1 = $week_day - GPS_config::$date_first_day;
                }
                else
                {
                    $offset1 = 7 - (GPS_config::$date_first_day - $week_day);
                }
            ...
            case 'this_week_full':
                ...
                $offset2 = 6 - $week_day + GPS_config::$date_first_day;
                $range['from'] = gmmktime(0, 0, 0, date('n', $time), date('j', $time) - $offset1, date('Y', $time));
    ...
    {
        if ($time)
            return $utc ? gmdate($this->date_format['php_d'] . ' ' . $this->date_format['php_t'], $time) : date(GPS_config::$php_date_format .
    ...
    protected function render_table_name($mode = 'list', $tag = 'h3 class="box-title"', $to_show = false)
    {
        $out = '';
        if ($this->is_title)
        {
            ...
            if (GPS_config::$can_minimize)
            {
                if ($to_show)
                    $out .= '<span class="gps-toggle-show gps-toggle-down"><i class="' . $this->theme_config('slide_down_icon') .
                        '"></i></span>';
                else
                    $out .= '<span class="gps-toggle-show gps-toggle-up"><i class="' . $this->theme_config('slide_up_icon') .
                        '"></i></span>';
            }
            $out .= $this->close_tag($tag);
        }
    ...
    {
        if (!GPS_config::$alt_encription_key)
        {
        ...
        $key = substr(GPS_config::$alt_encription_key, 0, $ks);
    ...
    {
        $_SESSION['lists']['gps_session'] = $data;
    }
    public static function export_session()
    {
        return $_SESSION['lists']['gps_session'];
    }
...
class GPS_postdata
{
    ...
    public function __construct($postdata, $gps)
    {
        $this->gps = $gps;
    ...
    {
        $fdata = $this->gps->_parse_field_names($name, 'GPS_postdata');
        foreach ($fdata as $key => $fitem)
        {
            $this->postdata[$key] = $value;
        }
        $this->gps->unlock_field($name);
        return $this;
    }
    ...
    public function get($name)
    {
        ...
        return /*new GPS_postdata_item*/ ($value);
    }
